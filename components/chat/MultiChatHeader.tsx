...
import { targetKey, type ChatTarget } from "@/lib/chat/targets";

import { CHANNEL_COLORS, type MultiPlatform } from "@/lib/chat/multiChat";
...
type MultiChatHeaderProps = {
    ...
    onPlatformChange: (value: MultiPlatform) => void;
    newChannel: string;
    onNewChannelChange: (value: string) => void;
    addError: string;
    channelPlaceholder: string;
    onAddChannel: (event: FormEvent) => void;
    onRemoveTarget: (target: ChatTarget) => void;
};
...
>
    ...
    {showAdd && targets.length < 4 && (
        ...
        value={newChannel}
        onChange={(event) =>
        onNewChannelChange(event.target.value)
    }
