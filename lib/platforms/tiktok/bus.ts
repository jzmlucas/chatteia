...
const GLOBAL_KEY = "__chatteiaTikTokChatBus";

function getBus(): TikTokChatBus {
    const globalObject = globalThis as typeof globalThis & {
        [GLOBAL_KEY]?: TikTokChatBus;
    };
...
export function subscribeTikTokChat(channel: string, listener: Listener) {
...
export function publishTikTokChat(message: UnifiedChatMessage) {
    const bus = getBus();
    ...
    for (const listener of listeners) {
        try {
            ...
            console.error("[TIKTOK] Listener error:", error);
        }
    }
}

export function hasTikTokListeners(channel: string): boolean {
    const bus = getBus();
    const normalized = channel.trim().replace(/^@/, "").toLowerCase();
    const listeners = bus.listeners.get(normalized);
